...
public class JZMediaExo extends UUMediaInterface implements Player.EventListener, VideoListener {
    ...
    public JZMediaExo(UUvideo jzvd) {
        super(jzvd);
    }
    ...
    public void prepare() {
        Log.e(TAG, "prepare");
        Context context = uuVideoView.getContext();
        ...
        mMediaHandler.post(() -> {
            ...
            Boolean isLoop = uuVideoView.jzDataSource.getLooping();
            if (isLoop) {
            ...
            simpleExoPlayer.setVideoSurface(new Surface(uuVideoView.textureView.getSurfaceTexture()));
        });
    ...
    public void seekTo(long time) {
        if (time != previousSeek) {
            ...
            previousSeek = time;
            uuVideoView.seekToInAdvance = time;
        }
    }
...
//        });
        ...
        handler.post(() -> {
            switch (playbackState) {
                ...
                case Player.STATE_READY: {
                    if (playWhenReady) {
                        uuVideoView.onStatePlaying();
                    } else {
                    }
                }
                break;
                case Player.STATE_ENDED: {
                    uuVideoView.onAutoCompletion();
                }
                break;
            }
        });
    ...
    public void onPlayerError(ExoPlaybackException error) {
        ...
        handler.post(() -> uuVideoView.onError(1000, 1000));
    }
